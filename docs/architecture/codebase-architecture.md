...
- text-targeted replacement implemented from PDF.js-extracted page text geometry for born-digital PDFs, with word-level viewer targets, shift-click contiguous multi-word selection, a direct inline viewer editor, and draggable/resizable snapping on-page region handles for faster Acrobat-like interaction
